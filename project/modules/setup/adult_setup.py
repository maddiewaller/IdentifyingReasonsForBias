...
def train():
    ...
    X_train, y_train = df_train_encode.loc[:, df_train_encode.columns != 'Probability'], df_train_encode['Probability']
    ...
    save_path = path.join(PROJECT_ROOT, 'processed_data/adult_census/test_data_encode.csv')
    ...
    save_path = path.join(PROJECT_ROOT, 'processed_data/adult_census/test_data.csv')
...
def find_similar_inds(k):
    ...
    X_test_encode = test_data_encode.loc[:, test_data_encode.columns != 'Probability']
    ...
    test_data['Probability'] = y_pred

    # clear the save directory of any previous results
    save_path = path.join(PROJECT_ROOT, 'processed_data/adult_census/adult_sim_inds')
    ...
    for index, row in test_data.iterrows():
        ...
        if row['Probability'] == " <=50K":
            ...
            filename = path.join(save_path, f"adult_inds_{index}.csv")
